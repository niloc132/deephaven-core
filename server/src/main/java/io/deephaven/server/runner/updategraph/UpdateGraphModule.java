...
public class UpdateGraphModule {
    ...
    public static UpdateGraph provideUpdateGraph() {
        PeriodicUpdateGraph updateGraph = PeriodicUpdateGraph.newBuilder(PeriodicUpdateGraph.DEFAULT_UPDATE_GRAPH_NAME)
                ...
                .existingOrBuild();
        // Need to clearly relocate this - can't call start() because we might be in a test. Is it more appropriate for the
        // builder to start for us? is there a case where we don't want the builder to start?
        // updateGraph.start();
        return updateGraph;
    }
}
