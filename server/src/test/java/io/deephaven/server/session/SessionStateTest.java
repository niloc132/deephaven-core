...
public class SessionStateTest {
     ...
     * <p>
     * The cancellation is done via {@link #cancelWithoutExportMapLock}, not {@code child.cancel()}: on this branch,
     * {@code cancel()}/{@code release()} always take the session's exportMap monitor before the export's own (see
     * {@code ExportObject#cancel}), so a second top-level {@code cancel()} call can no longer race a teardown that is
     * itself holding that same exportMap monitor for its whole cascade &mdash; it would simply block until the cascade
     * finishes, never landing in this window. The window this test targets is still real, though: it is the same one
     * {@code ExportObject#doExport} hits when it finds itself cancelled after being scheduled, and that path only ever
     * takes the export's own monitor, never the exportMap's.
     *
    ...
    private static Throwable cancelChildDuringTeardown(
        ...
        synchronized (child) {
            ...
            cancelWithoutExportMapLock(child);
        }
