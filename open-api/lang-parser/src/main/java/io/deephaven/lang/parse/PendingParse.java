...
public class PendingParse {
    ...
    /**
     * A ParseJob represents a unit of work for our parser. Note that we only ever want to parse / use the newest
     * ParseJob, and only when there is no new parse job incoming, as completion requests must only ever be issued when
     * the document is stable (if the document is changing, then any completion request is stale, as the user is typing
     * new code).
     */
    private final class ParseJob {

        /**
         * monaco-supplied version string. We use this to sanity check that we're not parsing an old document. We
         * _should_ do some testing that Ctrl+Z does not re-issue an old version identifier.
         */
        private final String version;
        /**
         * The text of the document. Monaco only sends us diffs of changes, and our parser is not (yet) incremental, so
         * we rebuild the whole document from the text of the previous state of the document.
         */
        ...
        /**
         * The final parsed result. Only non-null after a parse operation has completed. Note that we only ever read
         * this field inside a synchronized() block, so no need for volatile here.
         */
         ...
         *
         * @return true if parsing was successful, false if it failed with a parse exception, and null if we were
         *         interrupted.
         */
    ...
    /**
     * The thread doing the parse work. We (over)use this field to interrupt the parser, and as an object monitor to
     * un/pause our threads.
     */
    private final Thread parseThread;

    /**
     * Set whenever the document is mutated and we need to reparse. This is volatile so we can read the `String text`
     * inside it without acquiring any locks.
     */
    private volatile ParseJob targetState;
    /**
     * Set when a parse has completed either successfully or unsuccessfully. We use this to prevent re-parsing a
     * document so invalid that the parser blows up.
     */
    private ParseJob lastParseState;
    /**
     * Set when the parse has completed successfully. We only ever return a valid result when completedState ==
     * lastParseState == targetState, as this means "parse has completed successfully, and document has not been
     * mutated.
     */
    ...
    /**
     * Set to false when we know a document change is incoming, but a new parse has not been requested. Used to keep the
     * parser thread cleared while we are waiting.
     */
        ...
        while ((pending = awaitNext()) != null) {
            ...
            synchronized (parseThread) {
                ...
                if (Boolean.TRUE.equals(success)) {
                    // pending state was successfully parsed, signal that it has a completed document, and should not be
                    // run again
                    ...
                    // In this state, we will allow fallback to V1 parser if the document has not changed since we
                    // failed,
                    // otherwise, we will cancel all parse attempts until the document no longer causes the parser to
                    // blow up.
     ...
     * @param language The language of the document. Should be a value from
     *        {@link io.deephaven.lang.parse.api.Languages}
     * @param force A boolean to signal if we should forcibly parse, ignoring current state.
     */
    void requestParse(@NotNull final String version, @NotNull final String text, final String language,
            final boolean force) {
        ...
        synchronized (parseThread) {
            ...
            while (alive && valid && localTargetState != lastParseState) {
                ...
                try {
                    ...
                    throw new CompletionCancelled(); // calling code will catch this to fail-fast (does not allow
                                                     // fallback).
                }
            } // end while(){} loop
            ...
            if (lastParseState == targetState) {
                ...
                if (completedState == lastParseState) {
                    // The parse completed successfully, and the document is uptodate. Use the result to perform
                    // completion.
