    ...
    private static final class StreamDecoder extends CodedInputStream {
        ...
        private class SkippedDataSink implements RefillCallback {
            ...
            ByteBuffer getSkippedData() {
                if (byteArrayStream == null) {
                    return copy(buffer, lastPos, pos - lastPos);
                } else {
                    byteArrayStream.write(buffer, lastPos, pos);
                    return copy(byteArrayStream.toByteArray());
                }
            }
        }
        ...
        public ByteBuffer readByteBuffer() throws IOException {
            ...
            if (size <= (bufferSize - pos) && size > 0) {
                // Fast path: We already have the bytes in a contiguous buffer.
                ByteBuffer result = copy(Arrays.copyOfRange(buffer, pos, pos + size));
                pos += size;
                return result;
            }
            if (size == 0) {
            ...
            return copy(readRawBytesSlowPath(size, /* ensureNoLeakedReferences= */ true));
        }
...
//                return ensureNoLeakedReferences ? result.clone() : result;
    ...
    private static ByteBuffer copy(byte[] bytes) {
        return copy(bytes, 0, bytes.length);
    }
    private static ByteBuffer copy(byte[] bytes, int offset, int size) {
        ByteBuffer wrapper = ByteBuffer.allocate(size);
        wrapper.put(bytes, offset, size);
        return wrapper;
    }
}
